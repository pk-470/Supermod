...
class Submissions(
    ...
    async def my_subs(self, ctx: commands.Context, masterlist: Optional[str] = None):
        if await self.updating_check(ctx):
            return

        async def retrieve_sub(ctx: commands.Context, masterlist: str):
    ...
    async def update_sheet(
        self, ctx: commands.Context, masterlist: Optional[str] = None
    ):
        if await self.updating_check(ctx):
            return
